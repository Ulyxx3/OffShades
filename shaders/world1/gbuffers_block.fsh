...
/* OffShades — world1/gbuffers_block.fsh — End proxy */
#define WORLD_END
...
void main() {
    ...
    vec3 normal = normalize(v_normal); float bl = v_lm.x;
    vec4 spec = vec4(0.0);
#ifdef SPECULAR_MAPS
    spec = texture(specular, v_uv);
#endif
    gbuf0 = albedo; gbuf1 = vec4(albedo.rgb, 1.0);
    gbuf2 = vec4(encode_normal(normal), bl, 0.0);
